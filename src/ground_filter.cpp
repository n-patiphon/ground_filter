...
private:
	...
	bool		floor_removal_;
	double          radius_coeff_close_;
        double          radius_coeff_far_;
...
GroundFilter::GroundFilter() : node_handle_("~")
{
	...
	node_handle_.param("min_point", min_point_, 2);
	ROS_INFO("Min Points: %d", min_point_);
	node_handle_.param("clipping_thres", clipping_thres_, 6.00);
	ROS_INFO("Lower Clipping Threshold: %f", clipping_thres_);
	node_handle_.param("gap_thres", gap_thres_, 0.15);
	ROS_INFO("Point Gap Threshold: %f", gap_thres_);

	node_handle_.param("radius_coeff_close_", radius_coeff_close_, 0.2);
	ROS_INFO("First Coeficient for Optimal Radius Function: %f", radius_coeff_close_);
        node_handle_.param("radius_coeff_far_", radius_coeff_far_, 0.7);	
	ROS_INFO("Second Coeficient for Optimal Radius Function: %f", radius_coeff_far_);
	...
	initRadiusArray(optimal_radius_, sensor_model_);
}
        ...
        if (model == 32)
        {
                ...
                for (int i = 0; i < model; i++)
                {
                        if (i == 0)
                        {
                                ...
                                radius[i] = sensor_height_*(1.0/tan(theta) - 1.0/tan(theta + angle_res*M_PI/180.0));
                        }

                        if (i <= 12)
                        {
                                radius[i] = radius_coeff_close_*radius[i];
                        } else if (i <= 20) {
                                radius[i] = radius_coeff_far_*radius[i];
                        } else {
                                radius[i] = radius[20];
                        }
                }
		...
		for (int i = 0; i < model; i++)
                {
                        if (i < 32)
                        {
                                ...
                                if (i == 32)
                                {
                                        ...
                                        theta = theta*M_PI/180.0;
                                        radius[i] = sensor_height_*(1.0/tan(theta) - 1.0/tan(theta + 0.5*M_PI/180.0));
                                } else {
		...
		for (int j = vertical_res_ - 1; j >= 0; j--)
		{
			...
			if (j == 0)
			{
				...
				for (int m = unknown_index_size - 1; m >= 0; m--)
				{
					...
					point_distance_ = optimal_radius_[centroid_ring];
					if ((r_diff < point_distance_) || cluster_index_size == 0)
					{
						cluster_index[cluster_index_size] = unknown_index[m];
						cluster_index_size++;
						centroid = r0;
						centroid_ring = in_cloud_msg->points[unknown_index[m]].ring;
						if (m == 0)
						{
